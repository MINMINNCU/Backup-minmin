...
class modQlformHelper 
{
    ...
    function getArticleData($field)
    {
        ...
        if($article_id){
        	list($id,$title)=explode(':',$article_id);
        	return $id;
        }
    ...
    function getQuotationByDate($article_id)
    {
        $db=JFactory::getDbo();
        $db->setQuery('SELECT * FROM `#__quotation` WHERE `article_id`='.$article_id);
        $quotations=$db->loadObjectlist();

        return $quotations;
    }

    function getQuotationByPrice($article_id)
    {
        $db=JFactory::getDbo();
        $db->setQuery('SELECT * FROM `#__quotation` WHERE `article_id`='.$article_id.' ORDER BY `quotation` ASC');
        $quotations=$db->loadObjectlist();

        return $quotations;
    }

    function getItemAcceptStatus($article_id)
    {
        $db=JFactory::getDbo();
        $db->setQuery('SELECT `accept` FROM `#__quotation` WHERE `article_id`='.$article_id);
        $objs=$db->loadColumn();
    ...
    function getUser($user_id)
    {
        ...
        $db=JFactory::getDbo();
        $db->setQuery('SELECT * FROM `#__k2_users` WHERE `userID`='.$user_id);
        $user=$db->loadObjectlist();

        return $user;
    }

    function getAuthor($article_id)
    {

        $db=JFactory::getDbo();
        $db->setQuery('SELECT `created_by` FROM `#__k2_items` WHERE `id`='.$article_id);
