...
class ccommentModelComments extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$accused = $this->getUserStateFromRequest($this->context . '.filter.accused', 'filter_accused', '');
		$this->setState('filter.accused', $accused);
	...
	protected function getListQuery()
	{
		...
		$accused = $this->getState('filter.accused');
		if (is_numeric($accused))
		{
			switch ($accused) {
				case 0:
					$query->where('c.voting_no <= 0');
					break;

				case 1:
					$query->where('c.voting_no >=1');
					break;

				case 2:
					$query->where('c.voting_no >= 10');
					break;
